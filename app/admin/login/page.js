...
import { GoogleAuthProvider, signInWithPopup, signInWithEmailAndPassword, onAuthStateChanged, fetchSignInMethodsForEmail, createUserWithEmailAndPassword, sendPasswordResetEmail } from 'firebase/auth';
...
export default function AdminLoginPage() {
  ...
  async function isAllowedEmail(userEmail) {
    ...
    // Check allowed team emails in Firestore
    try {
      const team = await getPortfolioDoc('teamMembers');
      const allowed = team?.members || [];
      return allowed.some(m => m.email === userEmail && m.active !== false);
    } catch { return false; }
  }
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      // onAuthStateChanged handles redirect + access check
    } catch (e) {
  ...
  return (
    <div style={{ minHeight:'100vh', display:'flex', alignItems:'center', justifyContent:'center', background:'var(--bg-void)', position:'relative', zIndex:1 }}>
      <div style={{ background:'var(--bg-surface)', border:'1px solid var(--border-2)', borderRadius:'var(--radius-xl)', padding:'48px', width:'100%', maxWidth:'400px' }}>
        ...
        <div style={{ fontFamily:'Bebas Neue,sans-serif', fontSize:'2.5rem', color:'var(--text-1)', marginBottom:'40px', textAlign:'center', letterSpacing:'0.05em' }}>SHAKIL CMS</div>
        ...
        <div style={{ marginBottom:'20px' }}>
          ...
          <input
            ...
            onBlur={e=>e.target.style.borderColor='var(--border-2)'}
          />
          <button onClick={handleEmailSignIn} disabled={loading} style={{ width:'100%', padding:'13px', background:loading?'var(--bg-elevated)':'var(--accent)', color:loading?'var(--text-3)':'#fff', border:'none', borderRadius:'var(--radius-md)', fontFamily:'Outfit,sans-serif', fontWeight:700, fontSize:'0.9rem', cursor:loading?'not-allowed':'pointer', transition:'all 0.15s' }}>
        ...
        {/* Google */}
        <button onClick={handleGoogleSignIn} disabled={loading} style={{ width:'100%', padding:'13px', background:'var(--bg-elevated)', color:'var(--text-1)', border:'1px solid var(--border-2)', borderRadius:'var(--radius-md)', fontFamily:'Outfit,sans-serif', fontWeight:600, fontSize:'0.9rem', cursor:loading?'not-allowed':'pointer', display:'flex', alignItems:'center', justifyContent:'center', gap:'10px', transition:'border-color 0.15s' }}
        ...
        {info && (
          <div style={{ marginTop:'16px', padding:'12px', background:'var(--accent-muted)', border:'1px solid var(--accent-border)', borderRadius:'var(--radius-md)', fontFamily:'Outfit,sans-serif', fontSize:'0.85rem', color:'var(--accent)', textAlign:'center' }}>
            {info}
          </div>
        )}
      </div>
