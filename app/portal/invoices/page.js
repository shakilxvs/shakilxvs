...
function downloadInvoicePDF(invoice, client, siteConfig) {
  ...
  win.document.write(`<!DOCTYPE html><html lang="en"><head><meta charset="utf-8"/><title>${invoiceNum} — ${clientName}</title><style>*{box-sizing:border-box;margin:0;padding:0;}body{font-family:'Helvetica Neue',Arial,sans-serif;color:#111827;background:#f9fafb;font-size:14px;line-height:1.6;}@page{size:A4;margin:0;}@media print{body{background:#fff;-webkit-print-color-adjust:exact;print-color-adjust:exact;}.no-print{display:none!important;}.page{box-shadow:none!important;border-radius:0!important;margin:0!important;max-width:100%!important;}}.page{max-width:760px;margin:32px auto;background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.08);}.top-accent{height:5px;background:${accent};}.inner{padding:44px 52px;}.header{display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:40px;}.sender-name{font-size:20px;font-weight:800;color:${accent};}.sender-detail{font-size:12.5px;color:#6b7280;margin-top:2px;line-height:1.8;}.inv-label{font-size:10px;letter-spacing:2px;text-transform:uppercase;color:#9ca3af;text-align:right;font-weight:600;}.inv-num{font-size:24px;font-weight:800;color:#111827;text-align:right;margin-top:3px;}.inv-date{font-size:12px;color:#9ca3af;text-align:right;margin-top:4px;}.divider{height:1px;background:#f3f4f6;margin:0 0 32px;}.bill-row{display:grid;grid-template-columns:1fr 1fr;gap:32px;margin-bottom:32px;}.bill-label{font-size:10px;letter-spacing:2px;text-transform:uppercase;color:#9ca3af;font-weight:600;margin-bottom:8px;}.bill-name{font-size:15px;font-weight:700;color:#111827;margin-bottom:3px;}.bill-detail{font-size:13px;color:#6b7280;line-height:1.7;}.meta{display:flex;gap:32px;margin-bottom:32px;padding:16px 20px;background:#f9fafb;border-radius:8px;border:1px solid #f3f4f6;}.meta-lbl{font-size:10px;letter-spacing:1.5px;text-transform:uppercase;color:#9ca3af;font-weight:600;margin-bottom:4px;}.meta-val{font-size:14px;font-weight:700;color:#111827;}.status-pill{display:inline-block;padding:4px 14px;border-radius:100px;font-size:10px;font-weight:700;letter-spacing:1.5px;text-transform:uppercase;background:${statusBg};color:${statusColor};border:1.5px solid ${statusColor}55;}table{width:100%;border-collapse:collapse;border-radius:8px;overflow:hidden;border:1px solid #f3f4f6;margin-bottom:0;}thead th{padding:11px 16px;background:#f9fafb;font-size:10px;letter-spacing:1.5px;text-transform:uppercase;color:#9ca3af;font-weight:600;text-align:left;border-bottom:1px solid #f3f4f6;}thead th.r{text-align:right;}thead th.c{text-align:center;}.totals{display:flex;justify-content:flex-end;border-top:1px solid #f3f4f6;}.totals-inner{width:260px;padding:16px;}.tot-row{display:flex;justify-content:space-between;padding:5px 0;font-size:13.5px;color:#374151;}.tot-row.grand{border-top:2px solid #f3f4f6;margin-top:8px;padding-top:12px;font-size:16px;font-weight:800;color:#111827;}.tot-row.grand .v{color:${accent};}.pay-section{margin-top:28px;text-align:center;}.pay-btn{display:inline-block;padding:14px 36px;background:${accent};color:#fff;border-radius:8px;font-size:15px;font-weight:700;text-decoration:none;}.pay-sub{font-size:11.5px;color:#9ca3af;margin-top:8px;}.pay-url{font-size:12px;color:${accent};font-family:'Courier New',monospace;}.foot{margin-top:40px;padding-top:20px;border-top:1px solid #f3f4f6;display:flex;justify-content:space-between;align-items:center;}.foot-note{font-size:12px;color:#9ca3af;}.save-btn{position:fixed;bottom:24px;right:24px;padding:12px 24px;background:${accent};color:#fff;border:none;border-radius:8px;font-size:14px;font-weight:700;cursor:pointer;box-shadow:0 4px 16px rgba(35,77,194,0.35);}</style></head><body><div class="page"><div class="top-accent"></div><div class="inner"><div class="header"><div><div class="sender-name">${fromName}</div><div class="sender-detail">${fromEmail?fromEmail+'<br/>':''}${fromPhone?fromPhone+'<br/>':''}${fromAddress?fromAddress+'<br/>':''}${fromWebsite?fromWebsite:''}</div></div><div><div class="inv-label">Invoice</div><div class="inv-num">#${invoiceNum}</div><div class="inv-date">Issued ${now}</div></div></div><div class="divider"></div><div class="bill-row"><div><div class="bill-label">Billed To</div><div class="bill-name">${clientName}</div><div class="bill-detail">${clientCo?clientCo+'<br/>':''}${clientPhone?clientPhone+'<br/>':''}${clientAddr?clientAddr+'<br/>':''}${clientCityLine?clientCityLine+'<br/>':''}${clientCountry?clientCountry+'<br/>':''}${clientEmail?clientEmail:''}</div></div><div><div class="bill-label">Details</div><div class="bill-detail">${invoice.dueDate?'<strong>Due: </strong>'+invoice.dueDate+'<br/>':''}</div><span class="status-pill">${status}</span></div></div><div class="meta"><div class="meta-item"><div class="meta-lbl">Invoice #</div><div class="meta-val">${invoiceNum}</div></div><div class="meta-item"><div class="meta-lbl">Amount Due</div><div class="meta-val">${currency} ${subtotal.toLocaleString('en-US',{minimumFractionDigits:2,maximumFractionDigits:2})}</div></div>${invoice.dueDate?`<div class="meta-item"><div class="meta-lbl">Due Date</div><div class="meta-val">${invoice.dueDate}</div></div>`:''}<div class="meta-item"><div class="meta-lbl">Status</div><div class="meta-val">${status}</div></div></div><table><thead><tr><th>Description</th><th class="c">Qty</th><th class="r">Unit Price</th><th class="r">Amount</th></tr></thead><tbody>${itemRows}</tbody></table><div class="totals"><div class="totals-inner"><div class="tot-row"><span>Subtotal</span><span class="v">${currency} ${subtotal.toLocaleString('en-US',{minimumFractionDigits:2,maximumFractionDigits:2})}</span></div><div class="tot-row grand"><span>Total Due</span><span class="v">${currency} ${subtotal.toLocaleString('en-US',{minimumFractionDigits:2,maximumFractionDigits:2})}</span></div></div></div>${showPayBtn?`<div class="pay-section"><a class="pay-btn" href="${payUrl}">Pay Now — ${currency} ${subtotal.toLocaleString('en-US',{minimumFractionDigits:2,maximumFractionDigits:2})}</a><div class="pay-sub">Click the button above or visit:</div><div class="pay-url">${payUrl}</div></div>`:''}<div class="foot"><div class="foot-note">Thank you for your business!</div><div class="foot-note">${fromEmail}</div></div></div></div><button class="save-btn no-print" onclick="window.print()">⬇ Save as PDF</button></body></html>`);
  win.document.close();
}
...
export default function PortalInvoices() {
  ...
  useEffect(() => {
    try {
      ...
      // FIX: [] not [router] — the router reference can change on navigation,
      // which caused this effect to re-run and sometimes fire router.replace
      // mid-navigation, incorrectly redirecting away from this page.
