...
function AppCard({ app, featured = false }) {
  ...
  const iconSize = featured ? 100 : 72;

  const inner = (
    <div
      style={{ background:'var(--bg-surface)', border:'1px solid var(--border-2)', borderRadius:'var(--radius-lg)', padding: featured ? '28px' : '20px', display:'flex', flexDirection: featured ? 'row' : 'column', alignItems:'center', gap: featured ? '24px' : '14px', textAlign: featured ? 'left' : 'center', transition:'border-color 0.2s ease, transform 0.2s ease, box-shadow 0.2s ease', cursor: app.url ? 'pointer' : 'default', height:'100%' }}
    ...
    >
      ...
      <div style={{ flex:1, minWidth:0 }}>
        <div style={{ fontFamily:'Outfit, sans-serif', fontWeight:700, fontSize: featured ? '1.2rem' : '0.9rem', color:'var(--text-1)', marginBottom:'6px', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>
          {app.name}
        </div>
    ...
    <div style={{ minHeight:'100vh', paddingTop:'100px', paddingBottom:'80px', position:'relative', zIndex:1 }}>
      <div style={{ maxWidth:1280, margin:'0 auto', padding:'0 24px' }}>
        ...
        {!loading && apps.length > 0 && (
          <div style={{ fontFamily:'Space Mono,monospace', fontSize:'0.6rem', color:'var(--text-3)', marginBottom:'20px', letterSpacing:'0.08em' }}>
      ...
      <style>{`
        ...
        @media (max-width: 640px)  { .apps-grid { grid-template-columns: repeat(2, 1fr) !important; } }
      `}</style>
    </div>
  );
}
