...
function VerifiedBadge() {
  return (
    <span style={{
      ...
      color: '#2dd4bf', letterSpacing: '0.06em', textTransform: 'uppercase',
    ...
    >
      <div style={{ display: 'flex', gap: '2px' }}>
        {Array.from({ length: 5 }, (_, i) => (
          <Star key={i} size={14} fill={i < review.rating ? '#f5c518' : 'transparent'} color={i < review.rating ? '#f5c518' : 'var(--border-3)'} strokeWidth={1.5} />
        ))}
      </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
        <div style={{ width: 38, height: 38, borderRadius: '50%', background: 'linear-gradient(135deg, var(--accent), #1931AB)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'Bebas Neue,sans-serif', fontSize: '1.1rem', color: '#fff', flexShrink: 0 }}>
