...
/* ── Logo pill — responsive width, text fallback ─────────── */
function LogoPill({ src, alt, invert, size='section' }) {
  ...
  return (
    <div title={alt} style={{
      ...
      {!err && src
        ? <img src={src} alt={alt}
            style={{ height:h-12, width:'auto', maxWidth:80, objectFit:'contain', display:'block',
              filter:invert?'brightness(0) invert(1)':'none' }}
            onError={()=>setErr(true)}/>
        : <span style={{ fontFamily:'Space Mono,monospace', fontSize:'0.55rem', color:'rgba(255,255,255,0.45)',
            letterSpacing:'0.08em', textTransform:'uppercase', whiteSpace:'nowrap' }}>{alt}</span>
      }
    </div>
  );
}
...
function LogoRow({ logos=[], size='section' }) {
  ...
  return (
    <div style={{ display:'flex', gap:'6px', alignItems:'center', overflowX:'auto', flexWrap:'nowrap', scrollbarWidth:'none' }}>
...
function CopyBtn({ text }) {
  ...
  return (
    <button onClick={handle} style={{ display:'inline-flex', alignItems:'center', gap:'4px', padding:'6px 11px',
      background:copied?'rgba(35,77,194,0.12)':'var(--bg-void)', border:copied?'1px solid var(--accent-border)':'1px solid var(--border-2)',
      borderRadius:'var(--radius-sm)', color:copied?'var(--accent)':'var(--text-2)',
      fontFamily:'Space Mono,monospace', fontSize:'0.6rem', cursor:'pointer', flexShrink:0, whiteSpace:'nowrap' }}>
      {copied?<Check size={11}/>:<Copy size={11}/>}{copied?'Copied':'Copy'}
    </button>
  );
}

/* ── Bank details — every field its own copyable row ─────── */
function BankDetails({ bank }) {
  ...
  const rows = [
    ...
    ['State / Province', bank.state],
    ['Postal / ZIP',     bank.postalCode],
    ['Country',          bank.country],
  ].filter(([,v])=>v);
  return (
    <div style={{ background:'var(--bg-elevated)', border:'1px solid var(--border-1)', borderRadius:'var(--radius-lg)', overflow:'hidden', marginBottom:'14px' }}>
      {rows.map(([label,value],i)=>(
        <div key={label} style={{ display:'grid', gridTemplateColumns:'1fr auto', alignItems:'center', gap:'10px',
          padding:'10px 16px', borderBottom:i<rows.length-1?'1px solid var(--border-1)':'none' }}>
          <div style={{ minWidth:0 }}>
            <div style={{ fontFamily:'Space Mono,monospace', fontSize:'0.5rem', color:'var(--text-3)', textTransform:'uppercase', letterSpacing:'0.1em', marginBottom:'2px' }}>{label}</div>
...
function Accordion({ title, teaserLogos=[], isOpen, onToggle, children }) {
  return (
    <div style={{ background:'var(--bg-surface)', border:'1px solid var(--border-2)', borderRadius:'var(--radius-xl)', marginBottom:'10px', overflow:'hidden' }}>
      <div onClick={onToggle} style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'18px 22px', cursor:'pointer', gap:'12px', userSelect:'none' }}>
        <div style={{ minWidth:0 }}>
          <div style={{ fontFamily:'Bebas Neue,sans-serif', fontSize:'1.3rem', color:'var(--text-1)', letterSpacing:'0.04em', lineHeight:1, marginBottom:teaserLogos.length?'8px':'0' }}>{title}</div>
          {teaserLogos.filter(l=>l.active!==false).length>0 && <LogoRow logos={teaserLogos} size="section"/>}
        </div>
        <div style={{ transform:isOpen?'rotate(180deg)':'rotate(0deg)', transition:'transform 0.25s ease', color:'var(--text-3)', flexShrink:0 }}>
          <ChevronDown size={18}/>
        </div>
      </div>
      <div style={{ maxHeight:isOpen?'3000px':'0', opacity:isOpen?1:0, overflow:'hidden', transition:'max-height 0.32s ease, opacity 0.25s ease' }}>
...
function getPD(links,key){const r=links[key];if(!r)return{};if(typeof r==='string')return{link:r,paymentHandle:'',instructions:''};return r;}
