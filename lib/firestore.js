...
export async function getClientProjects(clientId) {
  try {
    const q = query(collection(db, 'portal_projects'), where('clientId', '==', clientId), orderBy('createdAt', 'desc'));
    const snap = await getDocs(q);
    return snap.docs.map(d => ({ id: d.id, ...d.data() }));
...
export async function getProjectTasks(projectId) {
  try {
    const q = query(collection(db, 'portal_tasks'), where('projectId', '==', projectId), orderBy('createdAt', 'asc'));
    const snap = await getDocs(q);
    return snap.docs.map(d => ({ id: d.id, ...d.data() }));
...
export async function getClientInvoices(clientId) {
  try {
    const q = query(collection(db, 'portal_invoices'), where('clientId', '==', clientId), orderBy('createdAt', 'desc'));
    const snap = await getDocs(q);
    return snap.docs.map(d => ({ id: d.id, ...d.data() }));
...
export async function getPortalMessages(clientId) {
  try {
    const q = query(collection(db, 'portal_messages'), where('clientId', '==', clientId), orderBy('sentAt', 'asc'));
    const snap = await getDocs(q);
    return snap.docs.map(d => ({ id: d.id, ...d.data() }));
...
export async function getProjectFiles(projectId) {
  try {
    const q = query(collection(db, 'portal_files'), where('projectId', '==', projectId), orderBy('createdAt', 'desc'));
    const snap = await getDocs(q);
    return snap.docs.map(d => ({ id: d.id, ...d.data() }));
