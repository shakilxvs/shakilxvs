...
// ─── Internal helper ─────────────────────────────────────────
// Strips the `id` field from data before writing. Firestore stores doc IDs
// as document keys, NOT as fields inside the doc. Storing `id` as a field
// corrupts later reads (the read returns the stored `id` instead of the
// real document key). Always run write payloads through this helper.
...
export async function setPortfolioDoc(docName, data) {
  try {
    const ref = doc(db, 'portfolio', docName);
    await setDoc(ref, { ...stripId(data), updatedAt: serverTimestamp() }, { merge: true });
    return true;
  } catch (e) {
...
export async function updateDocument(collectionName, id, data) {
  try {
    ...
    await updateDoc(ref, { ...stripId(data), updatedAt: serverTimestamp() });
    return true;
  } catch (e) {
...
export async function getFeaturedProjects() {
  // NOTE: Do NOT use compound where + orderBy here — it requires a Firestore
  // composite index and silently fails on deployments without one. We fetch
  // all projects, filter in JS, then sort in JS.
  try {
...
export async function rejectReview(pendingId, data) {
  try {
    ...
    batch.set(rejectedRef, { ...cleanData, rejectedAt: serverTimestamp() });
    batch.delete(doc(db, 'reviews_pending', pendingId));
    await batch.commit();
    return true;
  } catch (e) {
...
export async function restoreReview(rejectedId, data) {
  try {
    ...
    batch.set(pendingRef, { ...cleanData, status: 'pending', submittedAt: serverTimestamp() });
...
export async function unpublishReview(publishedId, data) {
  try {
    const { id: _id, ...cleanData } = data;
    const batch = writeBatch(db);
    const pendingRef = doc(collection(db, 'reviews_pending'));
    batch.set(pendingRef, { ...cleanData, status: 'pending', submittedAt: cleanData.submittedAt || serverTimestamp() });
    batch.delete(doc(db, 'reviews', publishedId));
    await batch.commit();
    return true;
  } catch (e) {
    console.error('unpublishReview:', e);
    throw e;
  }
}
...
export async function archiveMessage(id, data) {
  try {
    // Strip `id` field — never store doc ID as a field inside the doc itself.
    // Same pattern as approveReview/rejectReview/restoreReview.
    const { id: _id, ...cleanData } = data;
    const batch = writeBatch(db);
    const archivedRef = doc(collection(db, 'messages_archived'));
    batch.set(archivedRef, { ...cleanData, archivedAt: serverTimestamp() });
...
export async function getServices() {
  try {
    const ref = doc(db, 'portfolio', 'services');
    const snap = await getDoc(ref);
    return snap.exists() ? snap.data() : null;
  } catch (e) {
...
export async function addClient(data) {
  try {
    const ref = await addDoc(collection(db, 'clients'), {
      ...stripId(data), createdAt: serverTimestamp(),
    });
    return ref.id;
...
export async function updateClient(id, data) {
  try {
    await updateDoc(doc(db, 'clients', id), { ...stripId(data), updatedAt: serverTimestamp() });
    return true;
...
export async function getClientByUsername(username) {
  try {
    const q = query(collection(db, 'clients'), where('username', '==', username.toLowerCase()));
    const snap = await getDocs(q);
    if (snap.empty) return null;
    return { id: snap.docs[0].id, ...snap.docs[0].data() };
  } catch (e) { console.error('getClientByUsername:', e); return null; }
}
...
export async function getClientByEmail(email) {
  try {
    ...
    if (snap.empty) return null;
    return { id: snap.docs[0].id, ...snap.docs[0].data() };
  } catch (e) { console.error('getClientByEmail:', e); return null; }
}
...
export async function getClientById(id) {
  ...
  } catch (e) { console.error('getClientById:', e); return null; }
}

// ─── CRM — Portal Projects ────────────────────────────────────
export async function getClientProjects(clientId) {
  try {
    const q = query(collection(db, 'portal_projects'), where('clientId', '==', clientId));
    const snap = await getDocs(q);
    return snap.docs.map(d => ({ id: d.id, ...d.data() }))
      .sort((a,b) => {
  ...
  } catch (e) { console.error('getClientProjects:', e); return []; }
}

export async function getAllPortalProjects() {
  try {
    const q = query(collection(db, 'portal_projects'), orderBy('createdAt', 'desc'));
    const snap = await getDocs(q);
    return snap.docs.map(d => ({ id: d.id, ...d.data() }));
  } catch (e) { console.error('getAllPortalProjects:', e); return []; }
}

export async function addPortalProject(data) {
  try {
    const ref = await addDoc(collection(db, 'portal_projects'), { ...stripId(data), createdAt: serverTimestamp() });
    return ref.id;
  } catch (e) { console.error('addPortalProject:', e); throw e; }
}

export async function updatePortalProject(id, data) {
  try {
    await updateDoc(doc(db, 'portal_projects', id), { ...stripId(data), updatedAt: serverTimestamp() });
    return true;
  } catch (e) { console.error('updatePortalProject:', e); throw e; }
}

export async function deletePortalProject(id) {
  try {
    await deleteDoc(doc(db, 'portal_projects', id));
    return true;
  } catch (e) { console.error('deletePortalProject:', e); throw e; }
}

// ─── CRM — Tasks ──────────────────────────────────────────────
export async function getProjectTasks(projectId) {
  try {
    const q = query(collection(db, 'portal_tasks'), where('projectId', '==', projectId));
    const snap = await getDocs(q);
    return snap.docs.map(d => ({ id: d.id, ...d.data() }))
      .sort((a,b) => {
        const at = a.createdAt?.seconds ?? 0;
  ...
  } catch (e) { console.error('getProjectTasks:', e); return []; }
}

export async function addTask(data) {
  try {
    const ref = await addDoc(collection(db, 'portal_tasks'), { ...stripId(data), createdAt: serverTimestamp() });
    return ref.id;
  } catch (e) { console.error('addTask:', e); throw e; }
}

export async function updateTask(id, data) {
  try {
    await updateDoc(doc(db, 'portal_tasks', id), { ...stripId(data), updatedAt: serverTimestamp() });
    return true;
  } catch (e) { console.error('updateTask:', e); throw e; }
}

export async function deleteTask(id) {
  try {
    await deleteDoc(doc(db, 'portal_tasks', id));
    return true;
  } catch (e) { console.error('deleteTask:', e); throw e; }
}

// ─── CRM — Invoices ───────────────────────────────────────────
export async function getClientInvoices(clientId) {
  try {
    const q = query(collection(db, 'portal_invoices'), where('clientId', '==', clientId));
    const snap = await getDocs(q);
    return snap.docs.map(d => ({ id: d.id, ...d.data() }))
      .sort((a,b) => {
        const at = a.createdAt?.seconds ?? 0;
  ...
  } catch (e) { console.error('getClientInvoices:', e); return []; }
}

export async function getAllInvoices() {
  try {
    const q = query(collection(db, 'portal_invoices'), orderBy('createdAt', 'desc'));
    const snap = await getDocs(q);
    return snap.docs.map(d => ({ id: d.id, ...d.data() }));
  } catch (e) { console.error('getAllInvoices:', e); return []; }
}

export async function addInvoice(data) {
  try {
    const ref = await addDoc(collection(db, 'portal_invoices'), { ...stripId(data), createdAt: serverTimestamp() });
    return ref.id;
  } catch (e) { console.error('addInvoice:', e); throw e; }
}

export async function updateInvoice(id, data) {
  try {
    await updateDoc(doc(db, 'portal_invoices', id), { ...stripId(data), updatedAt: serverTimestamp() });
    return true;
  } catch (e) { console.error('updateInvoice:', e); throw e; }
}
...
export async function deleteInvoice(id) {
  try {
    ...
    return true;
  } catch (e) { console.error('deleteInvoice:', e); throw e; }
}

// ─── CRM — Portal Messages ────────────────────────────────────
export async function getPortalMessages(clientId) {
  try {
    const q = query(collection(db, 'portal_messages'), where('clientId', '==', clientId));
    const snap = await getDocs(q);
    return snap.docs.map(d => ({ id: d.id, ...d.data() }))
      .sort((a,b) => {
        const at = a.sentAt?.seconds ?? 0;
        const bt = b.sentAt?.seconds ?? 0;
        return at - bt;
      });
  } catch (e) { console.error('getPortalMessages:', e); return []; }
}

export async function getAllPortalMessages() {
  try {
    const q = query(collection(db, 'portal_messages'), orderBy('sentAt', 'desc'));
    const snap = await getDocs(q);
    return snap.docs.map(d => ({ id: d.id, ...d.data() }));
  } catch (e) { console.error('getAllPortalMessages:', e); return []; }
}

export async function sendPortalMessage(data) {
  try {
    const ref = await addDoc(collection(db, 'portal_messages'), {
      ...stripId(data),
      sentAt: serverTimestamp(),
      read: false,
    });
    return ref.id;
  } catch (e) { console.error('sendPortalMessage:', e); throw e; }
}

export async function markPortalMessageRead(id) {
  try {
    await updateDoc(doc(db, 'portal_messages', id), { read: true });
    return true;
  } catch (e) { console.error('markPortalMessageRead:', e); return false; }
}

// ─── CRM — Portal Files ───────────────────────────────────────
export async function getProjectFiles(projectId) {
  try {
    const q = query(collection(db, 'portal_files'), where('projectId', '==', projectId));
    const snap = await getDocs(q);
    return snap.docs.map(d => ({ id: d.id, ...d.data() }))
      .sort((a,b) => {
        const at = a.createdAt?.seconds ?? 0;
  ...
  } catch (e) { console.error('getProjectFiles:', e); return []; }
}

export async function addPortalFile(data) {
  try {
    const ref = await addDoc(collection(db, 'portal_files'), { ...stripId(data), createdAt: serverTimestamp() });
    return ref.id;
  } catch (e) { console.error('addPortalFile:', e); throw e; }
}

export async function deletePortalFile(id) {
  try {
    await deleteDoc(doc(db, 'portal_files', id));
    return true;
  } catch (e) { console.error('deletePortalFile:', e); throw e; }
}
...
export async function getDashboardStats() {
  try {
    const [
      clientsSnap, projectsSnap, invoicesSnap,
      messagesSnap, portalMsgSnap, reviewsSnap,
    ] = await Promise.all([
      getDocs(collection(db, 'clients')),
      getDocs(collection(db, 'portal_projects')),
      getDocs(collection(db, 'portal_invoices')),
      getDocs(collection(db, 'messages')),
      getDocs(collection(db, 'portal_messages')),
      getDocs(collection(db, 'reviews')),
    ]);
    ...
    const unreadPortal   = portalMsgSnap.docs.filter(d=>!d.data().read && d.data().from==='client').length;
    const unreadContact  = messagesSnap.docs.filter(d=>!d.data().read).length;
    ...
    return {
      ...
      unreadMessages:    unreadContact + unreadPortal,
...
export async function getProjectBySlug(slug) {
  try {
    ...
    if (snap.empty) return null;
    return { id: snap.docs[0].id, ...snap.docs[0].data() };
  } catch (e) { console.error('getProjectBySlug:', e); return null; }
}

// ─── FAQ ──────────────────────────────────────────────────
export async function getFAQ() {
  try {
    const ref = doc(db, 'portfolio', 'faq');
    const snap = await getDoc(ref);
    return snap.exists() ? (snap.data().items || []) : [];
  } catch (e) { console.error('getFAQ:', e); return []; }
}
...
export async function setFAQ(items) {
  try {
    ...
    return true;
  } catch (e) { console.error('setFAQ:', e); throw e; }
}
...
export async function getAllBlogPosts() {
  ...
  } catch (e) { console.error('getAllBlogPosts:', e); return []; }
}
...
export async function getPublishedBlogPosts() {
  try {
    ...
    return snap.docs.map(d => ({ id: d.id, ...d.data() }))
      .sort((a, b) => {
        const at = a.publishedAt?.seconds ?? a.createdAt?.seconds ?? 0;
        const bt = b.publishedAt?.seconds ?? b.createdAt?.seconds ?? 0;
        return bt - at;
      });
  } catch (e) { console.error('getPublishedBlogPosts:', e); return []; }
}
...
export async function getBlogPostBySlug(slug) {
  try {
    ...
    if (snap.empty) return null;
    return { id: snap.docs[0].id, ...snap.docs[0].data() };
  } catch (e) { console.error('getBlogPostBySlug:', e); return null; }
}
...
export async function addBlogPost(data) {
  try {
    ...
    return ref.id;
  } catch (e) { console.error('addBlogPost:', e); throw e; }
}

export async function updateBlogPost(id, data) {
  try {
    await updateDoc(doc(db, 'blog_posts', id), { ...stripId(data), updatedAt: serverTimestamp() });
    return true;
  } catch (e) { console.error('updateBlogPost:', e); throw e; }
}
...
export async function deleteBlogPost(id) {
  try {
    ...
    return true;
  } catch (e) { console.error('deleteBlogPost:', e); throw e; }
}

// ─── Log Feature — Settings ───────────────────────────────
// logSettings lives inside the `portfolio` collection as a
// single document (same pattern as hero, about, contact, faq, services).
// Fields: page_title, page_subtitle, page_enabled.
export async function getLogSettings() {
  try {
    const ref = doc(db, 'portfolio', 'logSettings');
    const snap = await getDoc(ref);
    return snap.exists() ? snap.data() : null;
  } catch (e) { console.error('getLogSettings:', e); return null; }
}

export async function setLogSettings(data) {
  try {
    const ref = doc(db, 'portfolio', 'logSettings');
    await setDoc(ref, { ...stripId(data), updatedAt: serverTimestamp() }, { merge: true });
    return true;
  } catch (e) { console.error('setLogSettings:', e); throw e; }
}

// ─── Log Feature — Posts ──────────────────────────────────
// Sort keys (featured DESC, post_date DESC) are computed in JS
// to avoid requiring a Firestore composite index. This matches
// the getFeaturedProjects pattern elsewhere in this file.
export async function getPublishedLogPosts() {
  try {
    const q = query(collection(db, 'logPosts'), where('published', '==', true));
    const snap = await getDocs(q);
    return snap.docs
      .map(d => ({ id: d.id, ...d.data() }))
      .sort((a, b) => {
        ...
        // Then by post_date DESC (newer first)
        const ta = a.post_date?.seconds ?? 0;
        const tb = b.post_date?.seconds ?? 0;
        return tb - ta;
      });
  } catch (e) { console.error('getPublishedLogPosts:', e); return []; }
}

export async function getAllLogPosts() {
  try {
    const snap = await getDocs(collection(db, 'logPosts'));
    return snap.docs
      .map(d => ({ id: d.id, ...d.data() }))
      .sort((a, b) => {
  ...
  } catch (e) { console.error('getAllLogPosts:', e); return []; }
}

export async function getLogPostById(id) {
  try {
    const snap = await getDoc(doc(db, 'logPosts', id));
    if (!snap.exists()) return null;
    return { id: snap.id, ...snap.data() };
  } catch (e) { console.error('getLogPostById:', e); return null; }
}

export async function addLogPost(data) {
  try {
    const ref = await addDoc(collection(db, 'logPosts'), {
      ...stripId(data),
      views: 0,
      created_at: serverTimestamp(),
    });
    return ref.id;
  } catch (e) { console.error('addLogPost:', e); throw e; }
}

export async function updateLogPost(id, data) {
  try {
    await updateDoc(doc(db, 'logPosts', id), {
      ...stripId(data),
      updatedAt: serverTimestamp(),
    });
    return true;
  } catch (e) { console.error('updateLogPost:', e); throw e; }
}

export async function deleteLogPost(id) {
  try {
    await deleteDoc(doc(db, 'logPosts', id));
    return true;
  } catch (e) { console.error('deleteLogPost:', e); throw e; }
}

// Public fire-and-forget view counter. Firestore rules enforce
// that the ONLY field this write can change is `views`, and only
// by +1, and only on published posts — so this is safe to call
// without auth.
export async function incrementLogPostViews(id) {
  try {
    const { increment } = await import('firebase/firestore');
    await updateDoc(doc(db, 'logPosts', id), { views: increment(1) });
    return true;
  } catch (e) {
    // Swallow — view count is non-critical, never surface errors
    return false;
  }
}
